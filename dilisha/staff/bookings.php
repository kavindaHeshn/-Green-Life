...
<head>
  ...
  <title>GreenLife Staff Dashboard - Bookings</title>
      ...
      <div class="container">
        ...
        if (isset($result) && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo '<td>';
                echo '<button class="send-mail-btn" data-email="' . htmlspecialchars($row['email']) . '" data-name="' . htmlspecialchars($row['name']) . '" aria-label="Send confirmation mail to ' . htmlspecialchars($row['name']) . '"><span><i class="fas fa-envelope"></i></span></button>';
                echo '</td>';
                echo '</tr>';
            }
            echo '</table>';
