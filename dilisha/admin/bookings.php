...
<head>
  ...
  <title>GreenLife Admin Dashboard - Bookings</title>
  ...
  <div class="flex-container">
      ...
      <div>
        ...
        <nav role="navigation" aria-label="Admin navigation">
          <ul>
            ...
            <li><a href="bookings.php" class="nav-link active" style="--index: 2;" aria-label="Manage bookings">Manage Bookings</a></li>
    ...
    <div class="main-content">
      ...
      <div class="container">
        ...
        <?php
        session_start(); // Start session for role-based access
        ...
        try {
          ...
          $sql = "SELECT id, name, email, package_name, preferred_date, preferred_time, notes, created_at FROM bookings ORDER BY created_at DESC";
          ...
          if ($result) {
            ...
            while ($row = $result->fetch_assoc()) {
              ...
              echo '<td>
                      <button class="send-mail-btn" data-email="' . htmlspecialchars($row['email']) . '" data-name="' . htmlspecialchars($row['name']) . '" aria-label="Send email to ' . htmlspecialchars($row['name']) . '"><i class="fas fa-envelope"></i></button>
                      <button class="delete-btn" data-id="' . htmlspecialchars($row['id']) . '" data-name="' . htmlspecialchars($row['name']) . '" aria-label="Delete booking for ' . htmlspecialchars($row['name']) . '"><i class="fas fa-trash"></i></button>
                    </td>';
              echo '</tr>';
            }
            echo '</table>';
  ...
  <script>
    ...
    document.querySelectorAll('.send-mail-btn').forEach(button => {
      button.addEventListener('click', () => {
        ...
        if (confirm(`Send confirmation mail to ${name}?`)) {
          fetch('send_mail.php', {
            method: 'POST',
            headers: {
